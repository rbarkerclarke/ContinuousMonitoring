...
ggpairs(patient_summary  %>% 
          ...
          mutate(across(c(stay_hrs, age, bmi,
                          mino2,minMAP, MAP_q0,`Duration of surgery (hours)`,pacu_los_hr),as.numeric)) %>% filter(`Duration of surgery (hours)`<20) %>% filter(pacu_los_hr<20), ggplot2::aes(colour=gender))
...
```{r}
for (i in unique(monitor_v$Study.ID)[1:10]){
  data = monitor_v %>% filter(Study.ID==i)
  HMM_model <- depmixS4::depmix(data$SPO2~1, nstates=3, ntimes=length(data$SPO2))
  HMM_fm <- fit(HMM_model)
  probs = posterior(HMM_fm)
  data$state = probs$state
  plot = ggplot(aes(x=time, y=SPO2, colour=as.factor(state)), data=data)+ geom_point()
  ggsave(filename = paste0(i,"spo2.png"), plot=plot)}
```


```{r}
model1=glm(formula = stay_hrs ~ mino2 +MAP_q0+o2_q0+o2_outlier + map_outlier + bmi+o2_q50+`Duration of surgery (hours)`+pacu_los_hr, data = patient_summary %>% mutate(across(c(stay_hrs, age, bmi,
                          mino2, MAP_q0,`Duration of surgery (hours)`,pacu_los_hr),as.numeric)))
...
```{r}
glm(formula = stay_hrs ~ MAP_q0+ o2_q0 + age + bmi+gender, data = patient_summary %>% mutate(stay_hrs = as.numeric(stay_hrs)))
```
